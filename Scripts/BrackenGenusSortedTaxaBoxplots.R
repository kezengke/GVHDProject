...
ttest_res<-read.table("ttest/BrackenGvNGenusResults.txt", sep = "\t", header = T, row.names = 1)
...
pdf("Plots/BrackenGenusGvNSortedTaxaBoxPlots(Ttest).pdf", width=14, height=7)
par(mfrow=c(2, 4))
par(mar=c(5,6,4,1)+.1)
...
ttest_res<-read.table("ttest/BrackenSvRGenusResults.txt", sep = "\t", header = T, row.names = 1)
ttest_res<-ttest_res[rownames(myT), , drop = F]
...
pdf("Plots/BrackenGenusSvRSortedTaxaBoxPlots(Ttest).pdf", width=14, height=7)
par(mfrow=c(2, 4))
par(mar=c(5,6,4,1)+.1)
