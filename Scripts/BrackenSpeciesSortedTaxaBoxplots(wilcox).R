...
wilcox_res<-read.table("wilcox/BrackenGvNSpeciesResults.txt", sep = "\t", header = T, row.names = 1)
...
pdf("Plots/BrackenSpeciesGvNSortedTaxaBoxPlots(Wilcox).pdf", width=14, height=7)
par(mfrow=c(2, 4))
par(mar=c(5,6,4,1)+.1)
...
wilcox_res<-read.table("wilcox/BrackenSvRSpeciesResults.txt", sep = "\t", header = T, row.names = 1)
wilcox_res<-wilcox_res[rownames(myT), , drop = F]
...
pdf("Plots/BrackenSpeciesSvRSortedTaxaBoxPlots(Wilcox).pdf", width=14, height=7)
par(mfrow=c(2, 4))
par(mar=c(5,6,4,1)+.1)
