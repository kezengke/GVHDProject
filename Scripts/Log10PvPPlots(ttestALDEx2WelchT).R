#log10 pvp for different statistics methods (ttest vs. ALDEx2 welch's ttest)
rm(list = ls())
library(scales)
...
pvpPlotFun <- function(result1, result2, testType1, testType2, condition, taxaLevel) {
  ...
  plot(result1$p_directed[result1[, 3]<0.05 & result2[, 2]<0.05], 
...
pdf("Plots/Log10PvalPlots(TtestAldex2WelchT).pdf", width=15, height=10)
par(mfrow=c(2,3))

ttestRES<-read.table("ttest/BrackenGvNPhylumResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenGvNPhylumALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "GvN", "Phylum")

ttestRES<-read.table("ttest/BrackenGvNGenusResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenGvNGenusALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "GvN", "Genus")

ttestRES<-read.table("ttest/BrackenGvNSpeciesResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenGvNSpeciesALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "GvN", "Species")

ttestRES<-read.table("ttest/BrackenSvRPhylumResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenSvRPhylumALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "SvR", "Phylum")

ttestRES<-read.table("ttest/BrackenSvRGenusResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenSvRGenusALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "SvR", "Genus")

ttestRES<-read.table("ttest/BrackenSvRSpeciesResults.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-read.table("ttest/BrackenSvRSpeciesALDEx2Results.txt", sep = "\t", header = T, row.names = 1)
aldex2WelchT<-aldex2WelchT[rownames(ttestRES), , drop = F]
pvpPlotFun(ttestRES, aldex2WelchT, "T-test", "ALDEx2WelchT", "SvR", "Species")
